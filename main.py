...
def select_topn_crypto_perps(top_n: int) -> List[Tuple[str, float, float, float]]:
    ...
    Filtert op crypto type of coin prefix 'CRYPTO:'.
    Score = log(dayNtlVlm) / (1 + 0.6*spreadPct)
    """
    try:
        # Haal universe + ctxs
    ...
    for i, asset in enumerate(universe):
        ...
        # Filter alleen echte crypto
        if asset_type != "CRYPTO" and not coin.upper().startswith("CRYPTO:"):
            continue

        seen_coins.add(coin)

        c = ctxs[i] if i < len(ctxs) else {}
        day_ntl = safe_float(c.get("dayNtlVlm", 0.0))
        mid = safe_float(c.get("midPx", 0.0))
        impact = c.get("impactPxs") or [mid, mid]
